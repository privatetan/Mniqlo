...
export default function SuperSelectionPage({ isFilterPanelOpen = false, onToggleFilterPanel, onCloseFilterPanel }: SuperSelectionPageProps) {
    ...
    const renderFilterControls = (mode: 'desktop' | 'mobile') => (
        <div className="space-y-4">
            <div className="flex items-start justify-between gap-3">
                ...
                <div className="filter-surface rounded-full px-3 py-2 text-[11px] font-medium text-slate-500">
            ...
            <div className="space-y-2">
                ...
                <div className="relative">
                    <input
                        ...
                        className="w-full h-11 pl-11 pr-4 bg-white/72 border border-white/70 rounded-full text-sm outline-none appearance-none shadow-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-600 transition-all"
                        value={searchQuery}
                ...
                <div className="space-y-2">
                    ...
                    <div className="filter-control-shell rounded-2xl px-1.5 py-1">
                        <select
                            ...
                            className="w-full text-xs px-3 py-2 pr-8 rounded-xl border-0 bg-transparent text-slate-600 outline-none appearance-none shadow-none focus:ring-0 focus:shadow-none cursor-pointer transition-all"
                        >
                    ...
                    {isFilterPanelOpen ? (
                        <div className="filter-surface rounded-[30px] px-5 py-4">
                            ...
                            <div className="mt-4 flex items-center justify-between gap-3">
                                <button
                                    ...
                                    className="filter-pill rounded-full px-4 py-2 text-xs font-semibold text-slate-600 transition-colors hover:text-teal-800"
                                >
                                    {t('filters.reset')}
                                </button>
                                <button
                                    ...
                                    className="filter-pill-active rounded-full px-4 py-2 text-xs font-semibold transition-transform hover:-translate-y-0.5"
                                >
                                    {t('filters.done')}
                                </button>
                            </div>
                        ...
                        >
                            <div className="min-w-0 flex-1">
                                <div className="flex flex-wrap gap-2">
                                    {summaryTokens.map((token) => (
                                        <span key={token} className="filter-pill rounded-full px-3 py-1 text-[11px] font-medium text-slate-600">
                                            {token}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="shrink-0 text-right">
                                <p className="text-sm font-semibold text-slate-700">{sortedProducts.length}</p>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
                        {sortedProducts.map(product => (
                            <div
                                key={product.code}
                                className="card rounded-[30px] p-0 overflow-hidden relative transition-none"
                            >
                                <div
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between text-[10px] font-medium pt-2 border-t border-slate-100/80 text-slate-400">
                                        <span>{t('fav.variants', { n: product.items.length })}</span>
                                        <div className="inline-flex items-center justify-center rounded-full px-2.5 py-1 bg-white/70 text-slate-400 border border-white/70">
                                            <svg
                                ...
                                {expandedCode === product.code && (
                                    <div className="mx-2 mb-2 rounded-[26px] px-4 pb-4 pt-2 bg-white/70">
                                        ...
                                        {expandedState?.code === product.code && (
                                            <div className="bg-slate-50/85 rounded-2xl border border-white/70 p-3">
            ...
            {isMounted && isFilterPanelOpen && createPortal(
                <div className="md:hidden fixed inset-0 z-[90] pointer-events-none">
                    ...
                    <div className="pointer-events-auto absolute inset-x-0 bottom-0 z-10 px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
                        <div
                            className="filter-surface touch-pan-y rounded-[30px] px-4 pb-4 pt-3 shadow-[0_-26px_60px_-28px_rgba(31,48,55,0.34)]"
                            style={{
                        ...
                        >
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-3">
                                <button
                                    type="button"
                                    onClick={resetFilters}
                                    className="filter-pill rounded-full px-4 py-3 text-sm font-semibold text-slate-600 transition-colors hover:text-teal-800"
                                >
                                    {t('filters.reset')}
                                </button>
                                <button
                                    type="button"
                                    onClick={closeFilterPanel}
                                    className="filter-pill-active rounded-full px-4 py-3 text-sm font-semibold transition-transform active:scale-[0.98]"
                                >
                                    {t('filters.done')}
                                </button>
                            </div>
